...
class TagEcho {
    ...
    switchClientMode(mode) {
        ...
        if (mode === 'external') {
            ...
            this.showNotification('Switched to Client View', 'info');
            
            // Switch to a safe tab for external users
            this.switchTab('analyzer');
        } else {
            userRole.textContent = 'System Administrator';
            this.showNotification('Switched to Admin View', 'info');
        }
...
<body>
    ...
    <div class="footer">
        <p>This report was generated by TAG Echo - Enterprise Platform</p>
    ...
    exportToCSV(data) {
        let csvContent = 'P2P Analytics Report\\n';
    ...
    exportToExcel(data) {
        ...
        excelContent += '<tr><td colspan="6"><strong>P2P Analytics Report</strong></td></tr>';
        excelContent += `<tr><td>Workspace:</td><td>${data.metadata.workspace}</td></tr>`;
        excelContent += `<tr><td>Period:</td><td>${data.metadata.period}</td></tr>`;
        excelContent += '<tr><td></td></tr>';
        excelContent += '<tr><td><strong>Vendor Name</strong></td><td><strong>Category</strong></td><td><strong>Total Spend</strong></td><td><strong>Orders</strong></td><td><strong>Avg Pay Time</strong></td><td><strong>Performance</strong></td></tr>';
    ...
    initializeUpdateIndicator() {
        this.updateIndicator = {
            element: document.getElementById('update-indicator'),
            commitElement: document.getElementById('commit-hash'),
            timeElement: document.getElementById('last-update'),
        ...
        this.updateIndicator.intervalId = setInterval(() => {
            this.updateVersionInfo();
        }, this.updateIndicator.refreshInterval);

        // Add click handler to force refresh
        this.updateIndicator.element.addEventListener('click', () => {
            this.updateVersionInfo(true);
        });
    }

    async updateVersionInfo(forceRefresh = false) {
        const indicator = this.updateIndicator;
        
        try {
            if (forceRefresh) {
                indicator.element.classList.add('loading');
                indicator.commitElement.textContent = 'Refreshing...';
                indicator.timeElement.textContent = 'Checking...';
            }
            ...
            const fileInfo = await this.getFileModificationInfo();
            
            // Update display
            indicator.commitElement.textContent = commitInfo.short;
            indicator.timeElement.textContent = fileInfo.lastModified;
            
            // Update tooltip with full commit info
            indicator.element.title = `Commit: ${commitInfo.full}\nBranch: ${commitInfo.branch}\nLast Modified: ${fileInfo.lastModified}`;
            
            indicator.element.classList.remove('loading', 'error');
            
        } catch (error) {
            console.error('Failed to update version info:', error);
            indicator.element.classList.add('error');
            indicator.element.classList.remove('loading');
            indicator.commitElement.textContent = 'Error loading';
            indicator.timeElement.textContent = 'Check failed';
        }
    }

    async getCommitInfo() {
        try {
            // Try to get git information from multiple sources
            const gitInfo = await this.fetchGitInfo();
            
            if (gitInfo) {
                return gitInfo;
            }
            
            // Fallback to simulated git info based on current time
            const now = new Date();
            const shortHash = this.generateShortHash();
            
            return {
                short: shortHash,
                full: `${shortHash} - Local development build`,
                branch: 'main',
                timestamp: now.toISOString()
            };
            
        } catch (error) {
            console.warn('Git info not available:', error);
            return {
                short: 'dev-build',
                full: 'Development build - Git info unavailable',
                branch: 'local',
    ...
    async fetchGitInfo() {
        ...
        const info = {
            ...
            author: 'tag-data',
            branch: 'main',
            timestamp: '2025-09-20T06:12:59-04:00'
        };
    ...
    generateShortHash() {
        // Generate a pseudo-git hash based on current time and page content
        const content = document.title + window.location.href + Date.now();
        let hash = 0;
        ...
        // Convert to hex and take first 7 characters (like git short hash)
        return Math.abs(hash).toString(16).substring(0, 7);
    }

    async getFileModificationInfo() {
        try {
            // Get the last modified time of the main files
            const files = ['index.html', 'script.js', 'styles.css'];
            const modificationTimes = [];
            
            // In a real application, you'd check actual file modification times
            // For now, we'll use the build time or current session info
            const lastModified = this.getLastModifiedTime();
            
            return {
                lastModified: this.formatRelativeTime(lastModified),
                timestamp: lastModified
            };
            
        } catch (error) {
            console.warn('File modification info not available:', error);
            return {
    ...
    getLastModifiedTime() {
        // Check if we have stored modification time
        const storedTime = localStorage.getItem('lastModified');
        if (storedTime) {
            return new Date(storedTime);
        }
        
        // Use current time as fallback and store it
        const now = new Date();
    ...
    destroyUpdateIndicator() {
        if (this.updateIndicator && this.updateIndicator.intervalId) {
            clearInterval(this.updateIndicator.intervalId);
        }
    }
}
